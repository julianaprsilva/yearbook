			...
			<form method="post" action="control/editar.php" enctype= "multipart/form-data">
				<fieldset>
					...
					<div>
						...
						<input size="35" maxlength="50" autofocus autocomplete="on" type="text" name="nome" id="nome" required value=<?php echo $participante->nomeCompleto;?>/>
					</div>
					<div>
						<label for="email">Email</label>
						<input size="35" maxlength="50" autofocus autocomplete="on" type="email" name="email" id="email" required value=<?php echo utf8_encode($participante->email);?>/>
					</div>
					<div>
						<label for="estado">Estado</label>
						<select id="estado" name="estado" autofocus required value=<?php echo $participante->objEstado->idEstado;?>>
							<option value="">Escolha um Estado</option>
							<?php foreach (listaEstados() as $estado): 
							       if($participante->objEstado->idEstado == $estado->idEstado) {?>
							       	<option value=<?php echo $estado->idEstado?> selected="selected"><?php echo utf8_encode($estado->nomeEstado)?>
							       	</option>
							       <?php }else?>
						  			<option value=<?php echo $estado->idEstado?>><?php echo utf8_encode($estado->nomeEstado)?></option>
						  	<?php endforeach;?>
						</select>			
					</div>
                    ...
                    <div>
                        ...
                        <textarea rows="4" cols="50" name="descricao" required><?php echo $participante->descricao;?></textarea>
                    </div>
                    <div>
